...
class Sina_Title_Widget extends Widget_Base{
	...
	protected function render() {
		...
		<div class="sina-title">
			...
			<?php if ( $data['title'] ): ?>
				<?php printf( '<%1$s class="sina-title-title">%2$s</%1$s>', sina_ext_escape_tags( $data['title_tag'] ), wp_kses_post($data['title'].$title_span) ); ?>
			<?php endif; ?>
			...
			<?php if ( $data['subtitle'] ): ?>
				<?php printf( '<%1$s class="sina-title-subtitle">%2$s</%1$s>', sina_ext_escape_tags( $data['subtitle_tag'] ), wp_kses_post($data['subtitle']) ); ?>
			<?php endif; ?>

			<?php if ( $data['desc'] ): ?>
				<div class="sina-title-desc"><?php echo wp_kses_post( $data['desc']); ?></div>
			<?php endif; ?>
		</div><!-- .sina-title -->
		<?php
	}
